...
class OriginalTransaction:
	...
	def from_dict(cls, t_dict: dict) -> 'OriginalTransaction':
		def build_subtransaction(s_dict: dict) -> SubTransaction:
			_category = Category(id=s_dict['category_id'], name=s_dict['category_name'])
			_payee = Payee(id=s_dict['payee_id'], name=s_dict['payee_name'], transfer_account_id=s_dict['transfer_account_id'])
			return SubTransaction(payee=_payee,
								  category=_category,
